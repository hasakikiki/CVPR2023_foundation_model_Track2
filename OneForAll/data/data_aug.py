...
def aug_vehicle_image():
    ...
    p.skew_tilt(probability=0.6, magnitude=0.4)
    p.rotate(probability=0.6, max_left_rotation=25, max_right_rotation=25)
    p.flip_left_right(probability=0.6)
    p.crop_centre(probability=0.6, percentage_area=0.7)
    p.random_erasing(probability=0.6, rectangle_area=0.2)
    p.sample(2 * len(os.listdir(clean_path)))
...
def merge_aug_result(data_type, config):
    ...
    with open(label_path, 'r') as f, open(save_path, 'w') as f_w:
        for line in tqdm(f):
            ...
            for aug_image in selected_image:
